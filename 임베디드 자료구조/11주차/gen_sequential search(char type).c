...
int charcmp(const void* a, const void* b) {			// a , b 크기 비교 , 자료형에 따라 형변환 해주어야됨
	return (*(char*)a - *(char*)b);
}

typedef int (*FCMP)(const void*, const void*);		// function pointer를 쉽게 호출하기 위한 typedef

int lv_search(void* key, void* base, int* num, int width, FCMP fcmp) {	// num(데이터길이), width 는 int형으로 사용, 찾기함수
	int i = 0;
...
int lv_insert(void* key, void* base, int* num, int width, FCMP fcmp) {		// num(데이터길이), width 는 int형으로 사용,  삽입함수
	memcpy(BASE((*num)++), key, width);			// a[(*num)++] = key; 맨뒤에 key 삽입

	return *num - 1;				// 맨 뒤값 , 인덱스 반환
}

int lv_delete(void* key, void* base, int* num, int width, FCMP fcmp) {		// num(데이터길이), width 는 int형으로 사용, 제거함수
	int p, i;
	if (*num > 0) {
...
int lfv_search(void* key, void* base, int* num, int width, FCMP fcmp) {	// num(데이터길이), width 는 int형으로 사용, 찾은후 앞으로 정렬 함수
	int i = 0, j;
	void* v;
...
void main() {
	...
	index = lv_search(&search, data, &nitem, sizeof(char), charcmp);						// search 찾기

	printf("E는 %d번째에 있습니다.\n", index + 1);				// 1더해줘야됨, 배열이니깐

	if (nitem < SIZE) lv_insert(&insert, data, &nitem, sizeof(char), charcmp);			// Y 삽입
	...
	printf("\n");
	lv_delete(&delete, data, &nitem, sizeof(char), charcmp);								// O 제거
	...
	printf("\n");
	lfv_search(&fsearch, data, &nitem, sizeof(char), charcmp);							//Y를 찾고 그 값을 맨앞으로 정렬
