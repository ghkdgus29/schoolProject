...
int lv_search(void* key, void* base, int* num, int width, FCMP fcmp) {	// num(데이터길이), width 는 데이터의 자료형에 따라 바꿔주어야 한다, 찾기함수
	int i = 0;
...
int lv_insert(void* key, void* base, int* num, int width, FCMP fcmp) {		// num(데이터길이), width 는 데이터의 자료형에 따라 바꿔주어야 한다, 삽입함수
	memcpy(BASE((*num)++), key, width);			// a[(*num)++] = key; 맨뒤에 key 삽입

	return *num - 1;				// 맨 뒤값 , 인덱스 반환
}

int lv_delete(void* key, void* base, int* num, int width, FCMP fcmp) {		// nubm(데이터길이), width 는 데이터의 자료형에 따라 바꿔주어야 한다, 제거함수
	int p, i;
	if (*num > 0) {
...
int lfv_search(void* key, void* base, int* num, int width, FCMP fcmp) {	// num(데이터길이), width 는 데이터의 자료형에 따라 바꿔주어야 한다, 찾은후 앞으로 정렬 함수
	int i = 0, j;
	void* v;
...
void main() {
	...
	for (i = 0; i < nitem; i++) data[i] = rand() % 10000;
	...
	printf("473은 %d번째에 있습니다.\n", index + 1);				// 1더해줘야됨, 배열이니깐

	if (nitem < SIZE) index = lv_insert(&insert, data, &nitem, sizeof(int), intcmp);		// 453 삽입
	else printf("Memory is full !!\n");							// 만약 데이터 공간이 꽉찼으면 삽입실패

	printf("마지막값 %5d, 데이터 길이 %d\n", data[index], nitem);

	
	index = lv_delete(&delete, data, &nitem, sizeof(int), intcmp);		// 473 제거

	printf("삭제된 473은 %d번째에 있었습니다.\n", index+1);			// 1더해줘야됨, 배열이니깐

	lfv_search(&fsearch, data, &nitem, sizeof(int), intcmp);		//453을 찾고 그 값을 맨앞으로 정렬
