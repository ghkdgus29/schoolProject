...
void print_critical_path(head net[], int V) {
	int i, j;
	node* t;

	for (i = 0; i < V; i++) {
		for (t = net[i].next; t; t = t->next) {
			if ((latest[t->vertex] - t->weight - earliest[i]) == 0) {
	...
	for (i = 0; i < V; i++) {
		for (j = 0; j < V; j++) {
			if (critical_path[i][j] == 0)
				critical_path[i][j] = 1000;
		}
	}

	floyd(critical_path, V);

	printf("\n\n");
	for (i = 0; i < V; i++) {
		for (j = 0; j < V; j++) {
			if (critical_path[i][j] == 1000)
				critical_path[i][j] = 0;
		}
	...
	for (i = 0; i < V; i++) {
		printf("%3c", int2name(i));
		for (j = 0; j < V; j++) printf("%3d", critical_path[i][j]);
		printf("\n");
	}
}
