...
void network_flow(int c[][MAX_NODE], int f[][MAX_NODE], int r[][MAX_NODE], int V, int S, int T) {			// flow , residual graph를 만드는 함수
	...
	while (get_augment_path(r, V, S, T)) {		// 1이면 경로가 더 있으니까 반복문 반복, 0이면 경로가 없다는 뜻이니 반복문 탈출
		...
		for (i = 1; path[i] >= 0; i++)		// path가 SINK부터 시작해 SOURCE에 다다르기 전까지 반복
...
int main(void) {
	int i;
	int V, E;
	fp = fopen("graph_matching.txt", "rt");
	input_directed_adjmatrix(Capacity, &V, &E);					// capacity graph 만들기

	printf("Capacity graph\n");
	print_directed_adjmatrix(Capacity, V);						// capacity graph 출력

	printf("\nFlow graph\n");
	network_flow(Capacity, Flow, Residual, V, SOURCE, SINK);
	print_directed_adjmatrix(Flow, V);							// Flow graph 출력


	printf("\nResidual graph\n");
	print_directed_adjmatrix(Residual, V);						// Residual graph 출력
	printf("\n\n");
