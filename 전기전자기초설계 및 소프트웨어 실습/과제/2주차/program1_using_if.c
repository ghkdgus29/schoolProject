...
int main() { // 메인함수를 실행한다.
	...
	while (1) { // 무한루프를 동작한다.
		...
		if (n == 1) temperature(); // 만약 n에 입력된 변수가 1일 경우 temperature() 함수를 실행한다.                         메뉴의 기능들은 if, else if, else 문을 이용하여 설정하였다.
		else if (n == 2) strlen(); // 만약 n에 입력된 변수가 2일 경우 strlen() 함수를 실행한다.							     if과 else if 은 조건을 넣어주어야 하며 else는 그외에 조건들을 
		else if (n == 3) cal();    // 만약 n에 입력된 변수가 3일 경우 cal() 함수를 실행한다.								 의미한다.
		else if (n == 4) count();  // 만약 n에 입력된 변수가 4일 경우 count() 함수를 실행한다.	
		else break;	               // 만약 n에 입력된 변수가 1,2,3,4를 제외한 나머지일 경우 무한루프를 빠져나간다.
	}
	return 0;  // 함수를 종료한다.
}